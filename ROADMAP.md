...
| W48 | **« Moins cher du secteur » par carburant** (suite de W47) : généraliser le relevé quotidien `RefreshPrix.gs` (boucle E85/Gazole/SP98, log `_PrixHistory` avec le bon `Type`, `SECTOR_BEST_TODAY` par carburant) + `Code.gs` `sectorPrices` + `secteur.js` pour suivre le carburant du sélecteur de la vue Carte | Rend la vue Carte utile pour Gazole/SP98 même sans historique de pleins · effort ~½ j (backend GAS) |
| W49 | **Alertes push par carburant** (suite de W47) : seuils indépendants E85/Gazole/SP98 dans ⚙️ Réglages ; `_PushSubs` étendu (colonnes seuil par carburant ou JSON) ; `WebPush.gs envoyerPushPrixBas` + `RefreshPrix.gs` notifient par carburant selon le seuil de chaque abonné | Étend l'alerte prix bas aux 3 carburants · effort ~½ j (web + GAS + push) |
