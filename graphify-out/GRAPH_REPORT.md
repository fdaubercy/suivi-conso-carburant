...
## Corpus Check
- 3 files · ~674,416 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2243 nodes · 3786 edges · 325 communities (149 shown, 176 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 159 edges (avg confidence: 0.79)
...
## Communities (325 total, 176 thin omitted)
...
Cohesion: 0.08
Nodes (38): APP_TOKEN (S6 token partagé GAS/VBA), GAS_URL, GOOGLE_CLIENT_ID (auth U7 GIS), _allRecords, buildHistoriqueCSV(), chargerHistorique(), CSV_COLS, _csvSep() (+30 more)
...
Cohesion: 0.12
Nodes (33): Flux d'authentification Google — GIS One-Tap → JWT → session localStorage, LWW parameters sync — last-write-wins reconciliation across app/Excel/GAS, auth-changed CustomEvent — émis à chaque (dé)connexion Google, authEnabled(), _avatarHtml(), _b64urlToJson(), _clearSession(), _dispatch() (+25 more)
...
Cohesion: 0.21
Nodes (27): DOM helper functions, _buildTypeToggle(), _updateHeaderBadges(), FUEL_KEYS, clearDraft(), onStationChange(), resetForm(), pickStation() (+19 more)

### Community 15 - "Formulaire et config Excel"
Cohesion: 0.13
Nodes (27): Historical price resolution — nearest-prior sector/station price for past fills, applyHistPriceToForm — fill form with historical price for past date (W60), applyHistPriceToForm(), _cacheKey(), escapeHtml(), _frDate(), getSectorMinForDate(), getSectorSeries() (+19 more)

### Community 16 - "Fonctionnalités de l'app"
Cohesion: 0.11
Nodes (25): CARTE_FUELS, computeStationAverages(), _defaultFuelForVehicle(), _ensureUserPos(), FUEL_TOKENS, _geocodeMissing(), _geocodeTried, initStationsMapInteractions() (+17 more)
...
Cohesion: 0.14
Nodes (22): submitForm, checkDuplicate(), _getClientId(), initVoiceKm(), onKmInput(), restoreDraft(), saveDraft(), submitForm() (+14 more)
...
Cohesion: 0.20
Nodes (19): OSM brand enrichment — single Overpass grouped query to name stations, showMap(), renderNearby(), enrichStationsBulk(), buildSearchClause(), buildStations(), geocodeAddress(), onAutreInput() (+11 more)

### Community 26 - "Statistiques et sparklines"
Cohesion: 0.11
Nodes (20): buildFuelSeries(), buildPrixSparkline(), buildSparklineSVG(), _ensureMarketData(), escHtmlLocal(), FUEL_PRICE_COL, initRapport(), initSparkToggles() (+12 more)
...
Cohesion: 0.19
Nodes (15): Geolocation localStorage cache TTL 1h (W31), OSM enrichment (enrichStationsBulk) — background station name resolution, Geo localStorage cache (GEO_CACHE_KEY, 1h TTL), geolocate(), highlightNearbyItem(), initNearbyList(), loadGeoCache(), saveGeoCache() (+7 more)
...
Cohesion: 0.12
Nodes (20): APP_TOKEN (token endpoints GAS S6), Code.gs (backend GAS), Gemini 2.0 Flash (scan ticket IA), Google Sheet (stockage données), Table GS_Pleins (Excel), Onglet _ImportGS (16+ colonnes), Last-write-wins par clé (paramètres), Onglet Parametres (source unique métier P1) (+12 more)
...
Cohesion: 0.14
Nodes (17): CHANGELOG — Suivi Conso Carburants, Axe Web app, 🎯 Backend (GAS), 🛠️ Dev / Outillage Claude, ROADMAP — Suivi E85, ☁️ Google Apps Script / Google Sheets, ✅ Idées déjà implémentées, 📋 Onglets Google Sheets (+9 more)

### Community 39 - "Installation et mise à jour"
Cohesion: 0.13
Nodes (16): GS_Pleins (table Power Query), modGraphiques.bas (dashboard graphiques), modSuiviVehicule.bas, _PrixHistory / PrixHistory (prix marche), v5.20.0.0, v5.21.0.0, GS_Pleins (table Power Query), Carte Rentabilité kit E85 (+8 more)

### Community 40 - "Affichage carte stations"
Cohesion: 0.24
Nodes (14): Stats cache TTL & freshness, buildStatsUrl(), getCachedServerStats(), getServerStats(), isFresh(), prewarmServerStats(), readStatsCache(), statsKey() (+6 more)
...
Cohesion: 0.13
Nodes (15): modSyncGS.bas (sync Google Sheets), v5.25.0.0, Empreinte CO2 E85 vs essence, Milestone v3.6.0.0 (S6 token + W37 Wrapped + W38), Milestone v5.25.0.0 (X46 santé sync), modSyncGS.bas (sync Excel↔GS), _SyncLog (journal de synchro), Bilan annuel Wrapped (wrapped.js) (+7 more)

### Community 43 - "Reprises de session"
Cohesion: 0.21
Nodes (12): viewchange event, persoAllowed(), _refreshVehBar(), t, chargerStations(), _gsStations, _historyStations, mergeHistoryStations() (+4 more)
...
Cohesion: 0.24
Nodes (13): modCarte.bas (cartes Excel), v5.22.0.0, v5.23.0.0, Carte alentour / stations les moins chères (cartealentour.js), Google Maps JS (cartographie), Milestone v5.23.0.0 (W82 cartes Google Maps), vba/modCarte.bas (3 cartes Excel : stations, proximité, itinéraire), Strip Attribute VB_Name avant set-module (sinon projet ne compile plus) (+5 more)
...
Cohesion: 0.21
Nodes (13): plein-added event, getAllRecords(), refreshAfterPlein(), buildCO2Tile(), buildPrediction(), _computePrediction(), computeStats(), getKitPrix() (+5 more)
...
Cohesion: 0.33
Nodes (12): CHANGELOG v5.13.0.0, Top 5 recommandés (priorités ROADMAP), Lesson: never CStr() a real date for re-parsing, Lesson: timeline SetFilterDateRange ignores fractional dates, Axe Excel, ROADMAP C9 — Service account Google, Top 5 recommandés en priorité, Top 5 recommandes en priorite (+4 more)
...
Cohesion: 0.27
Nodes (9): _fetchPricesNearUser(), initTypeToggle(), registerPriceCallback(), setType(), FUEL_CONFIG, Auth.gs (vérif idToken U7), GOOGLE_CLIENT_ID (OAuth Client), Comptes utilisateurs Google (U7) (+1 more)
...
Cohesion: 0.24
Nodes (10): BAN_API, Modèle CO2 E85 vs essence (tank-to-wheel), E85_RENTABLE_RATIO, FUEL_ANY, FUEL_CONFIG, FUEL_SELECT, PRIX_API, prix.js (+2 more)
...
Cohesion: 0.32
Nodes (7): ClassifyFilterDelta (rsNone/rsTargeted/rsFull), ComputeRebuildSignature(), CreerGraphiquesWeb (chemin rebuild complet), DebouncedRebuild, ReadLastSignature/WriteLastSignature, RebuildBlocks(blocks) — chemin ciblé, X43c — rebuild dashboard incrémental hybride
...
Cohesion: 0.38
Nodes (6): Tableau de bord Excel, Filtres natifs dashboard (segments véhicule/carburant + chronologie), BuildConsoBlock (gConso multi-vehicule), BuildPriceBlockMerged (gPrice multi-carburant), Perimetre pragmatique : Vehicule -> gConso seul, Filtre Periode (B9/B10, PERIODE_DEB/FIN)
...
Cohesion: 0.40
Nodes (6): Carte interactive Google Maps (W63) + clé Maps JS + Map ID, Clé Maps référent-restreinte échoue en file:// (gm_authFailure), Contrôle WebBrowser ActiveX retiré sur Win11/Office récent, W82 — Cartes Excel en Google Maps (MapEngineJs, repli Leaflet), Reprise fuel3 (conversion carte Leaflet → Google Maps), Reprise fuel4 (logos marqueurs X59c + #18 Google Maps en attente)
...
## Knowledge Gaps
- **796 isolated node(s):** `_comment`, `_doc`, `_scopes`, `scriptId`, `sheetId` (+791 more)
...
- **Why does `ROADMAP — Suivi E85` connect `Moteur de carte unifié` to `Mini-export reprise`, `Carte itinéraire`, `Champ coût et scan ticket`, `Résultat prix et rentabilité`, `Installation et mise à jour`, `Tuiles budget et CO2`, `Module ouverture classeur`, `Calculs surconso et CO2`, `Expert UserForms VBA`, `Config carburants et API`, `Carte Excel et clés Maps`, `Exemple spec UserForm`?**
  _High betweenness centrality (0.100) - this node is a cross-community bridge._
- **Why does `CLAUDE.md — Instructions Claude Code` connect `Carte Excel et clés Maps` to `Auth et paramètres app`, `README agent VBA`?**
  _High betweenness centrality (0.088) - this node is a cross-community bridge._
- **Why does `Suivi Conso Carburants — App` connect `Carte Excel et clés Maps` to `Service Worker et PWA`?**
