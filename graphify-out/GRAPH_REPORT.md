...
## Corpus Check
- 4 files · ~685,316 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2362 nodes · 4017 edges · 319 communities (146 shown, 173 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 201 edges (avg confidence: 0.78)
...
- **Pipeline prix carburant (config -> API prix)** — config_fuel_config, config_fuel_select, config_prix_api [INFERRED 0.85]
- **X44 modularisation VBA (lint + roadmap)** — changelog_x44_modularisation, changelog_x40_lint_vba, roadmap_x44 [INFERRED 0.75]
- **Sync PWA hors-ligne (SW + auth + feedback)** — changelog_w80_sync_hors_ligne, changelog_w82_sw_versioning, config_app_version [INFERRED 0.75]
...
Cohesion: 0.06
Nodes (58): Flux d'authentification Google — GIS One-Tap → JWT → session localStorage, LWW parameters sync — last-write-wins reconciliation across app/Excel/GAS, auth-changed CustomEvent — émis à chaque (dé)connexion Google, _avatarHtml(), _b64urlToJson(), _clearSession(), _dispatch(), ensureGis() (+50 more)
...
Cohesion: 0.14
Nodes (31): Geolocation localStorage cache TTL 1h (W31), OSM brand enrichment — single Overpass grouped query to name stations, OSM enrichment (enrichStationsBulk) — background station name resolution, showMap(), Geo localStorage cache (GEO_CACHE_KEY, 1h TTL), geolocate(), highlightNearbyItem(), initNearbyList() (+23 more)
...
Cohesion: 0.20
Nodes (29): _buildTypeToggle(), _fetchPricesNearUser(), initTypeToggle(), registerPriceCallback(), setType(), _updateHeaderBadges(), clearDraft(), onStationChange() (+21 more)
...
Cohesion: 0.10
Nodes (25): PRIX_API (data.economie.gouv.fr), gmapRenderCached(), CARTE_FUELS, computeStationAverages(), _defaultFuelForVehicle(), _ensureUserPos(), FUEL_TOKENS, _geocodeMissing() (+17 more)
...
Cohesion: 0.17
Nodes (25): viewchange event, checkDuplicate(), initVoiceKm(), onKmInput(), restoreDraft(), saveDraft(), getMaxKmForVehicule(), initStaticHandlers() (+17 more)
...
Cohesion: 0.10
Nodes (26): CHANGELOG — Suivi Conso Carburants, modSuiviVehicule.bas, v5.21.0.0, GS_Pleins (table Power Query), Suivi véhicule (modSuiviVehicule.bas, multi-véhicule), Axe Web app, 🎯 Backend (GAS), 🛠️ Dev / Outillage Claude (+18 more)

### Community 22 - "Reconstruction VBA sidebar"
Cohesion: 0.08
Nodes (25): Cas d'usage validés sur ce projet, 🎨 Charte & Workflow UI (design → code), CLAUDE.md — Suivi Conso Carburants, 🧰 Commandes, 🤝 Commandes déjà validées, 📚 Documentation obligatoire, 🎯 Déclenchement des skills (obligatoire), Délégation aux sous-agents (+17 more)
...
Cohesion: 0.18
Nodes (24): Offline sync queue — localStorage queue for failed GAS POSTs, plein-added event, submitForm, authEnabled(), promptLogin(), APP_TOKEN (S6 token partagé GAS/VBA), GAS_URL, GOOGLE_CLIENT_ID (auth U7 GIS) (+16 more)
...
Cohesion: 0.21
Nodes (19): Historical price resolution — nearest-prior sector/station price for past fills, refreshAfterPlein(), applyHistPriceToForm(), _cacheKey(), escapeHtml(), _frDate(), getSectorMinForDate(), getSectorSeries() (+11 more)
...
Cohesion: 0.21
Nodes (15): bestZoom(), hideMap(), initMapInteractions(), latLonToPx(), _renderGoogleMap(), _renderMap(), renderMiniMap(), showPinLabel() (+7 more)
...
Cohesion: 0.11
Nodes (18): Auth.gs (vérif idToken, SYNC_SECRET), auth.js (U7 comptes Google GIS), Code.gs (backend GAS doPost/doGet), Google Sheet (_ImportGS, Parametres), Identité utilisateur U7 (multi-utilisateur), parametres.js (sync LWW paramètres), Architecture technique, Col Q (Excel) — dirty flag · cols Q/R (GS) — S3/S5 (+10 more)
...
Cohesion: 0.20
Nodes (16): EnsureGSHeaders (X48 garde-fou en-têtes), modSyncCfg.bas — config sync Public Const, modSyncGS.bas (sync Google Sheets), modSyncJson.bas (helpers JSON/format), modSyncParametres (dédupliqué), Collision de nom Public entre modules VBA, modGraphiques.bas (~1500 l., Phase 3 à venir), modSyncEngine.bas (moteur SyncCore + import/export) (+8 more)

### Community 47 - "Modules sync VBA"
Cohesion: 0.17
Nodes (13): chargerStations(), _gsStations, _historyStations, mergeHistoryStations(), _renderStationOptions(), syncStationSiNouvelle(), Draft save/restore tests, tests/formulaire.test.js (+5 more)

### Community 48 - "Skill expert VBA"
Cohesion: 0.25
Nodes (15): CHANGELOG v5.13.0.0, X44 — modularisation VBA gros modules, X61 — dates FR des abscisses du Tableau de bord, Top 5 recommandés (priorités ROADMAP), Lesson: never CStr() a real date for re-parsing, Lesson: timeline SetFilterDateRange ignores fractional dates, Axe Excel, ROADMAP C9 — Service account Google (+7 more)

### Community 49 - "Expert UserForms VBA"
Cohesion: 0.19
Nodes (15): W80 — synchronisation hors-ligne debloquee, W82 — versionnage Service Worker / feedback badge hors-ligne, config.js (Configuration globale), APP_TOKEN, APP_VERSION, BAN_API, GAS_URL, Authentification Google U7 (GOOGLE_CLIENT_ID, GIS) (+7 more)
...
Cohesion: 0.23
Nodes (9): buildComparatifCSV(), CMP_COLS, computeVehicleComparison(), exportComparatifCSV(), initComparatifExport(), renderComparateur(), _priceBadge(), escHtml() (+1 more)
...
Cohesion: 0.20
Nodes (11): S5 — suppression d'un plein accessible utilisateur, commit.sh (gate Lint), Règle : retirer Attribute VB_Name avant set-module, Règle : Const mal placée = erreur compile-on-demand, Règle : garde anti-amputation graphify --update, tasks/lessons.md — Lessons Learned, scripts/check_vba_compile.py, vba/General.bas (versionné) (+3 more)
...
Cohesion: 0.25
Nodes (7): dismiss(), _hide(), initPWA(), triggerInstall(), Navigation par vues (W42), Suivi Conso Carburants — App, Vite (bundler/dev/build)
...
Cohesion: 0.52
Nodes (6): _close(), GMAPS_URL(), _onKey(), showStationPopup(), WAZE_URL(), haversine()
...
## Knowledge Gaps
- **822 isolated node(s):** `_comment`, `_doc`, `_scopes`, `scriptId`, `sheetId` (+817 more)
...
- **Why does `ROADMAP — Suivi E85` connect `Carte stations habituelles` to `Changelog snippets 4.3.0.5`, `Changelog déploiement 5.0`, `Carte itinéraire`, `API stats serveur`, `Changelog badge E85 et PWA`, `Contrôles UserForm`, `Reprises de session`, `Changelog suppressions`, `Changelog version 5`, `Skill expert VBA`, `Workflows CI et déploiement`, `Auth GAS et identité U7`?**
  _High betweenness centrality (0.106) - this node is a cross-community bridge._
- **Why does `CLAUDE.md — Instructions Claude Code` connect `Changelog badge E85 et PWA` to `Auth et paramètres app`, `Backend GAS et scan IA`, `Hub MAJ post-plein`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Why does `Suivi Conso Carburants — App` connect `Backend GAS et scan IA` to `Changelog badge E85 et PWA`?**
