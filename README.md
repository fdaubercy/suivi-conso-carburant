...
- carburant pas encore résolu à la saisie (SP95/E10/GPLc, cf. `HIST_FUELS`) → prix du jour conservé + note ; retour à aujourd'hui → prix live restauré. *(W61 : ces carburants sont désormais relevés dans `_PrixHistory` et synchronisés vers Excel ; leur remplissage auto à la saisie reste une évolution à venir — étendre `HIST_FUELS` + les `TOKENS` de `handleSectorPrices`.)*
