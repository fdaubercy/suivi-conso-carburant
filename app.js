/* ═══════════════════════════════════════
   ...
   v2.1.1.0 — Rayon de recherche manuelle configurable
═══════════════════════════════════════ */
...
let searchRadiusM      = 50000; // null = France entière
...
(function () {
  ...
  // Activer le bouton 50 km par défaut
  const defaultBtn = document.querySelector('.radius-btn[data-m="50000"]');
...
function setRadius(btn, metres) {
  searchRadiusM = metres; // null = France entière
  document.querySelectorAll('.radius-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  // Relancer si recherche déjà en cours
...
function setVehiculeStatus(cls, msg) {
  ...
  el.className = 'geo-status ' + cls; el.textContent = msg;
}
...
async function fetchOsmNameAround(lat, lon) {
  ...
  try {
    ...
    return name;
  } catch (e) { console.warn('[OSM] Erreur around :', e.message); return null; }
}
...
function _renderMap(uLat, uLon) {
  ...
  const mg = 0.008;
  const allLats = _mapStations.map(s => s.lat); if (uLat) allLats.push(uLat);
  const allLons = _mapStations.map(s => s.lon); if (uLon) allLons.push(uLon);
  ...
  _mapStations.forEach((s, i) => {
    ...
    html += `<div id="mapPin${i}" onclick="selectStationFromMap(${i})"
      onmouseenter="showPinLabel(${i})" ontouchstart="showPinLabel(${i})"
...
function setType(type) {
  ...
  fp.value = ''; fp.classList.remove('autofilled'); fp.placeholder = type === 'E85' ? '0.798' : '2.091';
  updateCout();
...
function onStationChange() {
  ...
  if (!isManual) {
    ...
    document.getElementById('fAutre').value = ''; setAutreStatus('', '');
  }
...
function onS98ManualEdit() {
  s98Autofilled = false;
  const el = document.getElementById('fPrixS98'); el.classList.remove('autofilled');
...
async function searchNearby(lat, lon, btn) {
  ...
  try {
    const resp = await fetch(odsUrl({
      ...
      select: 'adresse,ville,cp,e85_prix,sp98_prix,geom,services', limit: 40
    }));
    ...
    const stations = candidates.map((c, i) => {
      ...
      return { name, sub: stationSubLabel(c.r), dist: c.dist, lat: c.lat, lon: c.lon,
               e85: c.r.e85_prix, s98: c.r.sp98_prix,
...
function renderNearby(stations) {
  ...
  list.innerHTML = stations.map((s, i) => {
    ...
    const dist    = distNum == null ? '' : distNum < 1000 ? distNum + ' m' : (distNum/1000).toFixed(1) + ' km';
    ...
    return '<div class="nearby-item" id="nearbyItem' + i + '">'
      ...
      + '</div><a class="nearby-map-btn" href="https://www.google.com/maps/search/?api=1&query=' + s.lat + ',' + s.lon
      + '" target="_blank" rel="noopener" title="Voir sur la carte">🗺️</a></div>';
  }).join('');
...
function pickStation(name, lat, lon) {
  ...
  setGeoStatus('', ''); fetchPricesAtCoords(lat, lon, true);
}
...
/** Construit et trie le tableau de stations depuis les résultats bruts ODS */
function buildStations(results) {
  ...
  return results
    ...
    .map(r => {
      const c    = getCoords(r);
      const dist = (userLat && userLon) ? Math.round(haversine(userLat, userLon, c.lat, c.lon)) : null;
      const name = stationLabel(r);
      return { name, sub: stationSubLabel(r), dist, lat: c.lat, lon: c.lon,
               e85: r.e85_prix, s98: r.sp98_prix,
...
/**
 * Recherche avec le rayon sélectionné (searchRadiusM).
 * null = France entière (pas de clause distance).
 * Si position inconnue → pas de filtre proximité.
 * Si 0 résultat avec rayon → fallback France entière automatique.
 */
async function searchStationSuggestions(q) {
  try {
    const searchClause    = buildSearchClause(q);
    const hasPos          = userLat != null && userLon != null;
    const proximityClause = (hasPos && searchRadiusM != null)
      ? ` and distance(geom, geom'POINT(${userLon} ${userLat})', ${searchRadiusM}m)`
      : '';

    const url = PRIX_API + '?' + new URLSearchParams({
      where:    `${searchClause} and e85_prix is not null${proximityClause}`,
    ...
    // Aucun résultat dans le rayon → fallback France entière
    ...
    if (!data.results?.length) { setAutreStatus('err', 'Aucune station E85 trouvée pour cette ville.'); return; }
    const stations = buildStations(data.results);
    if (!stations.length) { setAutreStatus('err', 'Aucune station E85 trouvée pour cette ville.'); return; }

    const distLabel = proximityClause
      ? ' (dans ' + (searchRadiusM >= 1000 ? searchRadiusM/1000 + ' km' : searchRadiusM + ' m') + ')'
      : '';
    setAutreStatus('ok', stations.length + ' station(s) E85 trouvée(s)' + distLabel);
    renderNearby(stations);
...
async function searchStationSuggestionsNoProx(q, searchClause) {
  try {
    ...
    const stations = buildStations(data.results);
    if (!stations.length) { setAutreStatus('err', 'Aucune station E85 trouvée pour cette ville.'); return; }
    setAutreStatus('ok', stations.length + ' station(s) E85 trouvée(s) — France entière (aucune dans le rayon)');
    renderNearby(stations);
    showMap(userLat, userLon, stations.map((s, i) => ({ ...s, src:'nearby', srcIdx:i })));
...
function setFieldPrice(id, value, defaultPh) {
  ...
  if (v > 0) {
    ...
    el.classList.add('autofilled'); setTimeout(() => el.classList.remove('autofilled'), 6000);
...
function applyPricesResult(data) {
  ...
  s98Autofilled = !!(r.sp98_prix); updateCout();
  const found = [];
...
async function fetchPricesAtCoords(lat, lon, fallbackToUser = false) {
  ...
  for (const r of [500, 2000, 5000]) {
    try {
      const resp = await fetch(odsUrl({
        ...
        select: 'e85_prix,sp98_prix,adresse,ville,services', limit: 1
      }));
      if (!resp.ok) throw new Error('HTTP ' + resp.status);
      const data = await resp.json();
  ...
  if (fallbackToUser && userLat && userLon && haversine(lat, lon, userLat, userLon) > 100) {
    ...
    setFieldPrice('fPrixS98', null, '2.091'); updateCout();
...
async function fetchPricesNearUser() {
  userLat && userLon
    ? await fetchPricesAtCoords(userLat, userLon, false)
    : (setS98Status('info', 'Position inconnue — entrez le code postal :'), showCpSearch());
}
...
async function submitForm() {
  ...
  try {
    const json = await fetch(GAS_URL, {
      ...
      body: JSON.stringify({ date, type: currentType === 'E85' ? 'SuperEthanol E85' : 'Super 98', km, litres, prix, prixS98, station, vehicule })
    }).then(r => r.json());
    if (json.success) {
      showFeedback('success', 'Plein enregistré ✓', json.message || litres + ' L à ' + prix + ' €/L — ' + station);
      await syncStationSiNouvelle(station); resetForm();
...
function showFeedback(type, title, msg) {
  ...
  el.className = 'feedback ' + type; el.innerHTML = '<strong>' + title + '</strong>' + msg;
...
async function chargerStations() {
  ...
  try {
    ...
    csv.split('\n').map(l => l.trim().replace(/^"|"$/g, '')).slice(1)
